...
final class DexBootstrapPatcher {
    private static final String ACTIVITY = "Lcom/unity3d/player/UnityPlayerActivity;";
    private static final String BOOTSTRAP = "Lcom/unity3d/player/MobileEditorBootstrap;";
    ...
    static Result patch(File mainDex, File payloadDex, File outputDex) throws Exception {
        ...
        ClassDef bootstrap = findClass(payload, BOOTSTRAP);
        if (bootstrap == null) throw new IllegalStateException("MobileEditorBootstrap missing from payload dex");
        ...
        for (ClassDef classDef : main.getClasses()) {
            ...
            if (ACTIVITY.equals(type) || BOOTSTRAP.equals(type)) continue;
            classes.add(classDef);
        }
        classes.add(patchedActivity);
        classes.add(ImmutableClassDef.of(bootstrap));
