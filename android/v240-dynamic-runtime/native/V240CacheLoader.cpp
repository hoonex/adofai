...
std::string g_report =
        "nativeProbe=cache-post-bnm-abi\n"
...
void RunReadOnlyAbiProbe() {
    // Evidence-only. This stage resolves metadata handles after BNM is loaded but never
    // invokes managed code, writes managed state, creates objects, or installs hooks.
    ...
    out << "nativeProbe=cache-post-bnm-abi\n"
        << "nativeStage=post-bnm-read-only-abi\n"
        << "bnmLoadRequested=1\n"
        << "bnmLoadedCallback=1\n"
        << "probeComplete=1\n"
...
std::string CurrentReport() {
    if (!g_bnmLoadRequested.load(std::memory_order_acquire)) {
        return "nativeProbe=cache-post-bnm-abi\nnativeStage=post-bnm-read-only-abi\nbnmLoadRequested=0\nbnmLoadedCallback=0\nprobeComplete=0\ngameHooksInstalled=0\n";
    }
